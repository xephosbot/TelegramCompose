 ...
 * secret chats. In supergroups and channels, the user will not be able to return to the group on their
...
): Chat = this.sendFunctionAsync(TdApi.CreateNewBasicGroupChat(userIds, title,
...
/**
 * Suspend function, which returns RTMP URL for streaming to the chat; requires creator privileges.
 *
 * @param chatId Chat identifier.
 *
...
/**
 * Suspend function, which replaces the current RTMP URL for streaming to the chat; requires creator
 * privileges.
 *
 * @param chatId Chat identifier.
 *
...
/**
 * Suspend function, which changes the background in a specific chat. Supported only in private and
 ...
 * sharing it. Doesn&#039;t check botIsMember and botAdministratorRights restrictions. If the bot must
 * be a member, then all chats from getGroupsInCommon and all chats, where the user can add the bot,
 * are suitable. In the latter case the bot will be automatically added to the chat. If the bot must be
 * an administrator, then all chats, where the bot already has requested rights or can be added to
