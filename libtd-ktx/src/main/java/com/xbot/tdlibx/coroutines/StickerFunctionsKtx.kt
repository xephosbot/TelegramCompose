 ...
 * first. Only stickers belonging to a sticker set can be added to this list. Emoji stickers can&#039;t
 ...
 * from the list first. Only stickers belonging to a sticker set can be added to this list. Emoji
 ...
 * have the same format. For TGS stickers, uploadStickerFile must be used before the sticker is shown. 
 * 
...
suspend fun TelegramFlow.createNewStickerSet(
  ...
  source: String?
): StickerSet = this.sendFunctionAsync(TdApi.CreateNewStickerSet(userId, title, name, stickerFormat,
    stickerType, needsRepainting, stickers, source))

/**
 * Suspend function, which deleted a sticker set; for bots only.
 *
 * @param name Sticker set name.
 */
 ...
 * @param query Search query; a space-separated list of emoji or a keyword prefix. If empty, returns
...
/**
 * Suspend function, which changes the list of keywords of a sticker; for bots only. The sticker
...
/**
 * Suspend function, which changes the mask position of a mask sticker; for bots only. The sticker
...
suspend fun TelegramFlow.setStickerSetThumbnail(
  ...
  name: String?,
  thumbnail: InputFile?
) = this.sendFunctionLaunch(TdApi.SetStickerSetThumbnail(userId, name, thumbnail))

/**
 * Suspend function, which sets a sticker set title; for bots only.
 *
 * @param name Sticker set name.  
