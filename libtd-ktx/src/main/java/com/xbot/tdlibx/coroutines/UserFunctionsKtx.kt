 ...
 * specified.
 */
suspend fun TelegramFlow.reorderActiveBotUsernames(botUserId: Long, usernames: Array<String>?) =
    this.sendFunctionLaunch(TdApi.ReorderActiveBotUsernames(botUserId, usernames))
...
/**
 * Suspend function, which shares a user after pressing a keyboardButtonTypeRequestUser button with
 * the bot.
 *
 ...
 * @param onlyCheck Pass true to check that the user can be shared by the button instead of actually
 * sharing them.
 */
suspend fun TelegramFlow.shareUserWithBot(
  chatId: Long,
  messageId: Long,
  buttonId: Int,
  sharedUserId: Long,
  onlyCheck: Boolean
) = this.sendFunctionLaunch(TdApi.ShareUserWithBot(chatId, messageId, buttonId, sharedUserId,
    onlyCheck))
...
/**
 * Suspend function, which changes active state for a username of a bot. The editable username
 * can&#039;t be disabled. May return an error with a message &quot;USERNAMES_ACTIVE_TOO_MUCH&quot; if
 * the maximum number of active usernames has been reached. Can be called only if
 * userTypeBot.canBeEdited == true.
 *
 * @param botUserId Identifier of the target bot.  
 ...
 * owner privileges in the supergroup or channel. The editable username can&#039;t be disabled. May
 * return an error with a message &quot;USERNAMES_ACTIVE_TOO_MUCH&quot; if the maximum number of active
